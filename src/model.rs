...
fn split_triangles(splitter: f32,
                   distances: &[f32],
                   indices: &[u32],
                   faces: &[u32])
                   -> (Vec<u32>, Vec<u32>) {
...
fn build_leaf(kd_tree_nodes: &mut Vec<KdTreeNode>,
...
fn build_kd_tree<'r>(kd_tree_nodes: &mut Vec<KdTreeNode>,
                     new_indices: &mut Vec<u32>,
                     maxdepth: u32,
                     xdists: &'r [f32],
                     ydists: &'r [f32],
    ...
    build_kd_tree(&mut *kd_tree_nodes,
                  &mut *new_indices,
                  maxdepth - 1,
                  xdists,
                  ydists,
    ...
    let right_child_ix = build_kd_tree(&mut *kd_tree_nodes,
                                       &mut *new_indices,
                                       maxdepth - 1,
                                       xdists,
                                       ydists,
...
pub fn read_mesh(fname: &str) -> Mesh {
    ...
    let rootnode = build_kd_tree(&mut nodes,
                                 &mut new_indices,
                                 100,
                                 &xdists,
                                 &ydists,
...
fn read_polysoup(fname: &str) -> Polysoup {
    ...
    for line in reader.lines()
        ...
        if tokens[0] == "v" {
            ...
            let v = Vec3::new(tokens[1].parse().unwrap(),
            ...
            if tokens.len() == 4 || tokens.len() == 5 {
                ...
                println!("Polygon with {} vertices found. Ignored. Currently rustray only \
                          supports 4 vertices",
                         tokens.len() - 1);
            }
        } else if tokens[0] == "vt" {
