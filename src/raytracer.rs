use super::math3d::{Vec3, Mtx33, Ray, Triangle, HitResult, cosine_hemisphere_sample, rotate_to_up,
                    rotate_y};
use super::consts;
...
#[inline]
fn get_ray(horizontal_fov: f32,
           width: u32,
           height: u32,
           x: u32,
           y: u32,
           sample_jitter: (f32, f32))
           -> Ray {
...
fn get_rand_env() -> RandEnv {
    ...
    let mut hemicos_samples =
        Vec::with_capacity((consts::NUM_GI_SAMPLES_SQRT * consts::NUM_GI_SAMPLES_SQRT) as usize);

    for x in 0..consts::NUM_GI_SAMPLES_SQRT {
        for y in 0..consts::NUM_GI_SAMPLES_SQRT {
            let (u, v) = ((x as f32 + gen.gen::<f32>()) / (consts::NUM_GI_SAMPLES_SQRT as f32),
...
impl<'rand> Iterator for Stratified2dIterator<'rand> {
    ...
    fn next(&mut self) -> Option<(f32, f32)> {
        ...
        if self.n_samples == 1 {
            ...
            let r1 = (self.m_index as f32 + self.rnd.floats[offset % len]) /
                     (self.m_samples as f32);
            let r2 = (self.n_index as f32 + self.rnd.floats[(offset + 1) % len]) /
                     (self.n_samples as f32);
            self.offset += 2;
            Some((r1, r2))
        }
    }
...
#[inline]
fn trace_kd_tree(polys: &model::Polysoup,
    ...
    loop {
        ...
        match kd_tree_nodes[cur_node as usize] {
            ...
            model::KdTreeNode::KdNode(axis, splitter, right_tree) => {
                ...
                let (near, far) = if origin < splitter ||
                                     (origin == splitter && inv_dir_scalar >= 0.0) {
...
#[inline]
fn trace_kd_tree_shadow(polys: &model::Polysoup,
                        kd_tree_nodes: &[model::KdTreeNode],
                        kd_tree_root: u32,
                        r: &Ray,
                        inv_dir: Vec3,
...
fn trace_soup(polys: &model::Polysoup, r: &Ray) -> Option<(HitResult, u32)> {
    ...
    for tri_ix in 0..(polys.indices.len() as u32 / 3) {
        ...
        match (res, new_hit) {
            ...
            (Some((old_hit, _)), Some(hit)) if hit.t < old_hit.t && hit.t > 0.0 => {
                res = Some((hit, tri_ix));
            }
            _ => {}
        }
...
#[inline(always)]
fn direct_lighting<OcProbe>(lights: &[Light],
                            pos: Vec3,
                            n: Vec3,
                            view_vec: Vec3,
                            rnd: &RandEnv,
    ...
    where OcProbe: Fn(Vec3) -> bool
{
    ...
    for l in lights.iter() {
        ...
        let num_samples = match depth {
            0 => consts::NUM_LIGHT_SAMPLES,
            _ => 1,
        };    // do one tap in reflections and GI

        let rot_to_up = rotate_to_up((pos - l.pos).normalized());
        let shadow_sample_weight = 1.0 / (num_samples as f32);
        sample_disk(rnd, num_samples, |u: f32, v: f32| {
            // todo: stratify this

            // scale and rotate disk sample, and position it at the light's location
            let sample_pos = l.pos +
                             rot_to_up.transform(Vec3::new(u * l.radius, 0f32, v * l.radius));
        ...
        let light_contrib = if shadow_contrib == 0.0 {
            ...
            let atten = shadow_contrib * l.strength *
                        (1.0 / light_vec.length_sq() + specular * 0.05);
...
#[inline]
fn shade<OcProbe, ColProbe>(pos: Vec3,
                            n: Vec3,
                            n_face: Vec3,
                            r: &Ray,
                            color: Vec3,
          ...
          ColProbe: Fn(Vec3) -> Vec3
{
    ...
    let shading_normal = if consts::USE_SMOOTH_NORMALS_FOR_DIRECT_LIGHTING {
        n
    } else {
        n_face
    };

    let direct_light = direct_lighting(lights,
                                       pos,
                                       shading_normal,
                                       view_vec,
                                       rnd,
    ...
    let rcolor = if reflectivity > 0.001 {
        color_probe(reflection)
    } else {
        Vec3::new(0.0, 0.0, 0.0)
    };
    ...
    // Final gather GI
    let gi_normal = if consts::USE_SMOOTH_NORMALS_FOR_GI {
        n
    } else {
        n_face
    };
    ...
    if depth == 0 && consts::NUM_GI_SAMPLES_SQRT > 0 {
        ...
        ambient =
            ambient.scale(1.0 /
                          (((consts::NUM_GI_SAMPLES_SQRT * consts::NUM_GI_SAMPLES_SQRT) as f32) *
                           f32::consts::PI));
    }
...
#[inline]
fn trace_checkerboard(checkerboard_height: f32,
                      r: &Ray,
                      mint: f32,
                      maxt: f32)
                      -> (Option<Intersection>, f32) {
...
fn trace_ray(r: &Ray, mesh: &model::Mesh, mint: f32, maxt: f32) -> Option<Intersection> {
    ...
    match trace_result {
        Some((hit_info, tri_ix)) if hit_info.t > 0.0 => {
            ...
            let (i0, i1, i2) = (mesh.polys.indices[tri_ix + 0] as usize,
                                mesh.polys.indices[tri_ix + 1] as usize,
                                mesh.polys.indices[tri_ix + 2] as usize);

            // interpolate vertex normals...
            let n = (mesh.polys.normals[i0].scale(hit_info.barycentric.z) +
...
fn trace_ray_shadow(r: &Ray, mesh: &model::Mesh, mint: f32, maxt: f32) -> bool {
    ...
    // trace against scene
    trace_kd_tree_shadow(&mesh.polys,
                         &mesh.kd_tree.nodes,
                         mesh.kd_tree.root,
                         r,
                         r.dir.recip(),
...
fn get_color(r: &Ray,
    ...
    let default_color = Vec3::new(clamp(1.0 - theta * 4.0, 0.0, 0.75) + 0.25,
                                  clamp(0.5 - theta * 3.0, 0.0, 0.75) + 0.25,
                                  theta);    // fake sky colour
        ...
        for column in 0..width {
            ...
            for (u, v) in Stratified2dIterator::new(&rnd, sample_grid_size, sample_grid_size) {
                ...
                shaded_color = shaded_color +
                               get_color(r, mesh, &lights, &rnd, 0.0, f32::INFINITY, 0);
            }
            shaded_color = gamma_correct(shaded_color.scale(sample_coverage_inv *
                                                            sample_coverage_inv))
                               .scale(255.0);
            let pixel = (clamp(shaded_color.x, 0.0, 255.0),
...
fn generate_raytraced_image_single(mesh: model::Mesh,
                                   horizontal_fov: f32,
                                   width: u32,
                                   height: u32,
                                   sample_grid_size: u32,
    ...
    PixelIterator::new(width, height)
        .map(|pixel| {
            let mut shaded_color = Vec3::new(0.0, 0.0, 0.0);
...
fn generate_raytraced_image_multi(mesh: model::Mesh,
                                  horizontal_fov: f32,
                                  width: u32,
                                  height: u32,
                                  sample_grid_size: u32,
    ...
    for i in 0..((height / step_size) + 1) {
        let ttd = TracetaskData {
            // The data required to trace the rays.
            mesh: mesh.clone(),
...
pub fn generate_raytraced_image(mesh: model::Mesh,
                                horizontal_fov: f32,
                                width: u32,
                                height: u32,
                                sample_grid_size: u32)
    ...
    let lights = vec![Light::new(Vec3::new(-3.0, 3.0, 0.0),
                                 10.0,
                                 0.3,
                                 Vec3::new(1.0, 1.0, 1.0))]; //,
  //Light::new(Vec3::new(0f32, 0f32, 0f32), 10f32, 0.25f32, Vec3::new(1f32,1f32,1.0f32))];
    ...
    if num_tasks > height {
        num_tasks = height
    };   // We evaluate complete rows, there is no point in having more tasks than there are rows.
    match num_tasks {
        1 => {
            generate_raytraced_image_single(mesh,
                                            horizontal_fov,
                                            width,
                                            height,
                                            sample_grid_size,
        ...
        n => {
            generate_raytraced_image_multi(mesh,
                                           horizontal_fov,
                                           width,
                                           height,
                                           sample_grid_size,
