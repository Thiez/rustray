...
impl Mul<Mtx33> for Mtx33 {
    ...
    fn mul(self, rhs: Mtx33) -> Mtx33 {
        ...
        Mtx33 {
            r0: Vec3::new(self.r0.dot(&rhs.r0),
                          self.r0.dot(&rhs.r1),
                          self.r0.dot(&rhs.r2)),
            r1: Vec3::new(self.r1.dot(&rhs.r0),
                          self.r1.dot(&rhs.r1),
                          self.r1.dot(&rhs.r2)),
            r2: Vec3::new(self.r2.dot(&rhs.r0),
                          self.r2.dot(&rhs.r1),
                          self.r2.dot(&rhs.r2)),
        }
    }
...
impl Ray {
    ...
    pub fn intersect(&self, t: &Triangle) -> Option<HitResult> {
        ...
        if t < 0.0 {
            ...
            Some(HitResult {
                barycentric: Vec3::new(b1, b2, 1.0 - b1 - b2),
                t: t,
            })
        }
    }
    ...
    pub fn bb_check(&self, max_dist: f32, bbox: BoundingBox) -> bool {
        ...
        let (tx1, tx2, ty1, ty2, tz1, tz2) = ((bbox.min.x - self.origin.x) * inv_dir.x,
...
pub fn rotate_to_up(up_vec: Vec3) -> Mtx33 {
    let perp = if up_vec == Vec3::new(0f32, 1f32, 0f32) {
...
pub fn rotate_y(theta: f32) -> Mtx33 {
    ...
    let st = theta.sin();
    Mtx33 {
        r0: Vec3::new(ct, 0f32, st),
        r1: Vec3::new(0f32, 1f32, 0f32),
        r2: Vec3::new(-st, 0f32, ct),
    }
}
...
pub fn cross_test() {
    ...
    let expected = Vec3::new((y1 * z2) - (y2 * z1),
                             (z1 * x2) - (z2 * x1),
                             (x1 * y2) - (x2 * y1));
